...
class salog:
    ...
    def enableControllog(self,enable):
        if enable:
            ...
            if self._handle_control:
                ...
                self._handle_control.flush()
                self._handle_control.close()
    ...
    def enableFilelog(self,filename):
        if len(filename)>0:
            if self._handle_file:
                ...
                self._handle_file.flush()
                self._handle_file.close()
    ...
    def _output(self,level,msg):
        ...
        s="[%s] %04d-%02d-%02d %02d:%02d:%02d.%03d (0x%04X):%s"%(level,now.year,now.month,now.day,now.hour,now.minute,now.second,now.microsecond/1000,(thread.get_ident()>>8)&0xffff,msg)
        fn=logging.NOTSET
        if level==NONE:
            fn=None
        elif level==TRACE:
            fn=self._obj.debug
            sys.stderr.write('\033[0;37m')
        elif level==DEBUG:
            fn=self._obj.debug
            sys.stderr.write('\033[0;34m')
        elif level==INFORMATION:
            fn=self._obj.info
